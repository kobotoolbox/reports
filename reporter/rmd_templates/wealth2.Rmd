...
pie_chart(respondents, 'NationalQuintile') + ggtitle('Respondents by wealth quintile')
...
## Urban quintile results

These are the urban quintile results for these respondents:

```{r, echo=FALSE, results='asis'}
density <- as.data.frame(table(respondents$UrbanQuintile) / nrow(respondents) * 100)
names(density) <- c('Quintile', 'Percentage')
...
```{r, echo=FALSE}
pie_chart(respondents, 'UrbanQuintile') + ggtitle('Respondents by wealth quintile')

bottom_two <- sum(density$Percentage[density$Quintile %in% c(1, 2)])
```

The table above shows you the percentage of your respondents that were in each urban wealth quintile.
